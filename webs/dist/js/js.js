...
function animateClass() {
  setTimeout(function () {
    clList.forEach(function (e, t) {
      for (
        var o = document.querySelectorAll(e), a = 0, s = o.length;
        a < s;
        a++
      ) {
        var n = o[a],
          i = n.getBoundingClientRect();
        i.top - window.innerHeight < 0 && i.bottom > -50
...
$(document).ready(function () {
  ...
  $(".filter-wrapper .filter-elements li").on("click", function () {
    ...
    if (!$this.hasClass("active")) {
      ...
      $(".our-projects")
        .find(".box-wrapper[data-filter-prop*='" + getValueOfTab + "']")
        .each(function (i) {
      ...
      $(".our-projects")
        ...
        .show()
        .siblings(
          ".box-wrapper:not([data-filter-prop*='" + getValueOfTab + "'])"
        )
        .slideUp();
    } else {
  ...
  $(".filter-wrapper .filter-elements li").each(function () {
    ...
    $this
      ...
      .text(
        "(" +
          $(".our-projects").find(
            ".box-wrapper[data-filter-prop*='" + getValueOfTab + "']"
          ).length +
          ")"
      );

    /* populating dropdown values for browse by projects starts */
    $(".browse-select-dropdown select").append(
      "<option value=" + getValueOfTab + ">" + $this.text() + "</option>"
    );
  ...
  $(".browse-select-dropdown select").on("change", function () {
    if ($(this).val() == "none") {
      ...
      $(".our-projects")
        .find(".box-wrapper[data-filter-prop='" + $(this).val() + "']")
        .show()
        .siblings(
          ".box-wrapper:not([data-filter-prop='" + $(this).val() + "'])"
        )
        .slideUp();
    }
