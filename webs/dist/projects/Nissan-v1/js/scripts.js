...
var GL = {
  ...
  init: function() {
    ...
    ($('body').attr('data-view', 'desktop'), $('section.mobile').remove()) :
    ($('body').attr('data-view', 'mobile'), $('section[data-sec="18.1"]').remove());
  ...
  changeBg: function() {
    $('section').each(function() {
      if ($(this).has('data-sec')) {
        ...
        (
          $(this).find('.fp-bg').css({
            'background-image': 'url("images/bg768/' + $(this).attr('data-sec') + '.jpg") ',
        ...
        (
          ($(this).find('.fp-bg').css({
              'background-image': 'url("images/bg1900/' + $(this).attr('data-sec') + '.jpg")',
              'background-repeat': ' no-repeat',
              'background-position': 'center',
  ...
  openPopup: function(tag) {
    ...
    if (tag.hasClass('redButton')) {
      ...
      if (DEFAULTVARS.winWidth > 768) {
        (tag.hasClass('is-active')) ?
        (
          $('html,body').removeClass('overflowH'),
          tag.removeClass('is-active'),
          tag.parents('section').find('.redPop').remove()
        ) :
        (
          $('.mbPopup').removeAttr('style').attr('class', 'mbPopup').find('.popupBody').removeAttr('style'),
          $('.mbPopup .scroll').html('').removeAttr('style'),
          ...
          tag.parents('section').append('<div class="redPop">' + data + '</div>'),
          GL.popPos(tagPopPos, tagPos, tag)
        )
      } else {
        $('body,html').addClass('overflowH'),
    ...
    if (tag.attr('class') == 'playButton') {
      ...
      $('.mbPopup .scroll').html('<iframe width="' + fWid + '" height="' + fHei + '" src="' + tag.attr('data-vidUrl') + '?rel=0&amp;showinfo=0&amp;autoplay=1" frameborder="0" allowfullscreen allow="autoplay; encrypted-media"></iframe>');
      $('.mbPopup').css({
    ...
    $('.para').fullpage({
      ...
      afterLoad: function(anchorLink, index) {
        ...
        if ($(nxtSec).hasClass('darkBg')) {
         ...
         $('section[data-move="' + index + '"]').find('.centerCont').css({ 'transform': 'translateY(0)', 'opacity': 1, 'transition': 'opacity 0.5s 0.3s ease, transform 1s ease' })
