var secOnePlaneInit,
    ...
    joinBg2, joinBG2Init, aboutnetworkData, nAboutAnimData, nAboutAnim, whyJoinJAnimData, whyJoinJAnim, aboutnetwork, membershipBg2Anime, membershipBg2Data;
            ...
            if (value == 0) {
                ...
                setTimeout(function() {
                    ...
                    $('.video').css({
                        ...
                        left: 0,
                        background: '#000'
                    }).find('video').attr('src','dist/video/teaser.mp4').css({ opacity: 1, height: ($(window).height() - 10), width: (winWidth - 10) }).attr('controls', '');
                    $('header,#sectionTitle1,#social,.scrollDownHolder').fadeOut();
                    $('.video .close').fadeIn();
                    if(vid.readyState > 0)
                    {
                        // setTimeout(function(){
                    ...
                    $(document).on('click', '.video .close', function() {
                        ...
                        $('.video video').css({ opacity: 0.1, width: 'auto', height: '100%' }).removeAttr('controls').attr('src','dist/video/teaser-thumb.mp4')
                        $('header,#sectionTitle,#social,.scrollDownHolder').fadeIn();
                        $('.video .close').fadeOut();
                        if(vid.readyState > 0)
                        {
...
if($('body').attr('id') == 'membership')
{
    ...
    $(window).on('scroll resize', function() {
        ...
        // Fixed Table header
            ...
            // var innerBoxScrollTop = $('.enterprise-list.orig').offset().top;
            // console.log(docScrolloffset > innerBoxScrollTop);
    ...
    $(window).on('resize scroll', function() {
        ...
        $('.content .row .introText').each(function() {
            ...
            if ($(this).isInViewportCustom() && !$(this).parents('.row').eq(0).hasClass('complete') && $(this).parents('.row').index() == 1) {
                var currRow = $(this).parents('.row');
                setTimeout(function(){
                    TweenMax.to($(currRow).find('.clmn_icon').find('.yellowBox'), 0.4, { transform: 'scale(1, 1) translate(30px, 0)', ease: Power1.easeInOut });
            ...
            if ($(this).isInViewportCustom() && !$(this).parents('.row').eq(1).hasClass('complete') && $(this).parents('.row').index() == 2) {
                ...
                setTimeout(function(){
                    if(!$('#aboutnetwork').hasClass('complete'))
                    {
                        ...
                        // aboutnetwork.addEventListener('DOMLoaded', function(){
                            ...
                            if(!mobile)
                            {
                                $('.scrollbar-rail').scrollbar({
                                    onScroll: function(){
                                        // if($('.jsonChange').eq(0).isInViewportCustom()) {
                                        //     aboutnetwork.playSegments([[114, 240]], true);
                                        // }
                                        // if($('.jsonChange').eq(1).isInViewportCustom()) {
                                        //     aboutnetwork.playSegments([[240, 310]], true);
                                        // }
                                        // if($('.jsonChange').eq(2).isInViewportCustom()) {
                                        //     aboutnetwork.playSegments([[310, 400]], true);
                                        // }
                                        // console.log($('.scroll-wrapper').find('.jsonChange').length);
                                        // $('.scroll-wrapper .jsonChange').each(function(){
                                            // $('.scroll-wrapper').isInViewport({ tolerance: 10, viewport: $(this).css({color: '#f00'})})
                                        ...
                                        // $('.scroll-wrapper').find('.jsonChange').isInViewport({ tolerance: 0, viewport: $('.scroll-wrapper') }).each(function(){
                                        //     if($(this).index() == 0)
                                        //     {
                                        //         console.log(0);
                                        //         aboutnetwork.playSegments([[114, 240]], true);
                        ...
                        // });
                        // document.getElementsByClassName('jsonChange')[0].addEventListener('mouseenter', function(){
                        //     aboutnetwork.playSegments([[115, 200]], true);
                        // });
                    }
                }, 1000)
